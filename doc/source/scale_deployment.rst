...
handled, the API and engine services can be overloaded. In those scenarios, in
order to increase the system performance, it can be helpful to run multiple
load-balanced APIs and engines.
...
RabbitMQ's default round-robin distribution of messages will act as the load
balancer for multiple engines.
...
Thus, a request sent from the CLI looks like:
    ...
    2. The HAProxy server, acting as a load balancer, redirects the call to an
       API instance;
    3. The API server sends messages to RabbitMQ, and the engine servers pick up
       messages from the RabbitMQ queue in round-robin fashion.
...
Running multiple APIs requires that you create as many copies of this file as
