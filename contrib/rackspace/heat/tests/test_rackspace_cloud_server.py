...
class CloudServersTest(HeatTestCase):
    ...
    def test_server_validate_too_many_personality_files(self):
        ...
        self.assertEqual("The personality_files property may not contain "
                         "greater than four entries.", str(exc))
        self.m.VerifyAll()

    def test_server_validate_disallowed_personality_files(self):
        stack_name = 'srv_val'
        (t, stack) = self._setup_test_stack(stack_name)
        ...
        exc = self.assertRaises(exception.StackValidationFailed,
                                server.validate)
        self.assertEqual("The personality_files property may not contain a "
    ...
    def test_rackconnect_deployed(self):
        ...
        self.m.ReplayAll()
        scheduler.TaskRunner(server.create)()
        self.assertEqual(server.action, 'CREATE')
        self.assertEqual(server.status, 'COMPLETE')
        self.m.VerifyAll()
