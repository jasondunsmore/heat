...
"""
    ...
    # Only 4 personality files are allowed here, so update property
    # description
    properties_schema = copy.deepcopy(server.Server.properties_schema)
    properties_schema[server.Server.PERSONALITY_FILES].description = \
        _('A map of files to overwrite on the server upon boot. Keys are '
          'file names and values are the file contents (either as a string '
          'or as a file-like object). A maximum of four entries is allowed, '
    ...
    def distro(self):
        ...
        # TODO(jason): Update this to work with bootable volumes
        # (image would be None)
        if self.properties.get(self.IMAGE) is not None:
    ...
    def has_userdata(self):
        ...
        if user_data is not None or self.metadata != {}:
            return True
        else:
            return False
    ...
    def validate(self):
        ...
        if self.properties.get(self.IMAGE) is not None \
           ...
           and self.has_userdata:
            msg = _("user_data is not supported for image %s.") \
                % self.properties.get(self.IMAGE)
            raise exception.StackValidationFailed(message=msg)

        # The validation of PERSONALITY_FILES is slightly different
        # than in Server
        personality_files = self.properties.get(self.PERSONALITY_FILES)
        if personality_files is not None:
            if "/root/.ssh/authorized_keys" in personality_files:
                msg = _("The personality_files property may not contain a key "
                        "of \"/root/.ssh/authorized_keys\"")
                raise exception.StackValidationFailed(message=msg)
            elif len(personality_files) > 4:
                msg = _("The personality_files property may not contain "
                        "greater than four entries.")
                raise exception.StackValidationFailed(message=msg)

        # It's important that this runs last because otherwise the
        # error message could be wrong if len(personality_files) > 5
    ...
    def _personality_files(self):
        ...
        personality_files = {"/root/.ssh/authorized_keys":
                             '\n'.join(public_key)}
        property_files = self.properties.get(self.PERSONALITY_FILES)
        if property_files:
            personality_files.update(property_files)
