...
class BackupConfig(resource.Resource):
    ...
    properties_schema = {
        ...
        START_TIME_AM_PM: properties.Schema(
            ...
            constraints=[
                constraints.AllowedValues(['AM', 'PM', 'null']),
            ],
            default="AM",
            required=True,
            update_allowed=True),
    ...
    ATTRIBUTES = (
        BACKUP_CONFIG_ID, BACKUP_AGENT_ID, BACKUP_CONFIG_DETAILS
    ) = ('backup_config_id', 'backup_agent_id', 'backup_config_details', )
    attributes_schema = {
        BACKUP_CONFIG_ID: attributes.Schema(
            _("Backup configuration ID.")
        ),
    ...
    def handle_create(self):
        ...
        if not backup_agent_id:
            raise Exception("Backup agent not found")
    ...
    def validate(self):
        ...
        if self.properties[self.FREQUENCY] in ["Manually", "Hourly", "Daily"]:
