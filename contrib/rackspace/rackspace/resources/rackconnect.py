...
class PublicIP(resource.Resource):
    ...
    attributes_schema = {
        PUBLIC_IP_V4: attributes.Schema(
            _("Rackconnected server public IP."),
    ...
    def _check_status(self, public_ip):
        ...
        if public_ip.status == 'ACTIVE':
            # save the public_ip
            self.data_set(self.PUBLIC_IP_V4, public_ip.public_ip_v4)
            return True
    ...
    def _resolve_attribute(self, name):
        if name == self.PUBLIC_IP_V4:
            return self.data().get(self.PUBLIC_IP_V4, '')
