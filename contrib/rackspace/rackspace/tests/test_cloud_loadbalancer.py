...
class FakeLoadBalancer(object):
    ...
    def session_persistence(self, *args, **kwargs):
        pass
...
class LoadBalancerTest(common.HeatTestCase):
    ...
    def test_update_half_closed(self):
        rsrc, fake_loadbalancer = self._mock_loadbalancer(self.lb_template,
                                                          self.lb_name,
                                                          self.expected_body)
        self.m.ReplayAll()
    ...
    def test_update_session_persistence_add(self):
        ...
        self.m.StubOutWithMock(fake_loadbalancer, 'session_persistence')
        fake_sp_mgr = self.m.CreateMockAnything()
        fake_loadbalancer.session_persistence().AndReturn(fake_sp_mgr)
        fake_sp = self.m.CreateMockAnything()
        fake_sp_mgr.resource(persistenceType='SOURCE_IP').AndReturn(fake_sp)
        fake_sp_mgr.add(fake_sp).AndReturn(None)
    ...
    def test_update_session_persistence_delete(self):
        ...
        fake_sp_mgr = self.m.CreateMockAnything()
        fake_loadbalancer.session_persistence().AndReturn(fake_sp_mgr)
        fake_sp_mgr.delete().AndReturn(None)
