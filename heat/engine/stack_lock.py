...
class StackLock(object):
    ...
    def acquire(self):
        ...
        lock_type, lock_engine_id = db_api.stack_lock_create(self.stack.id,
                                                             engine_id)
        if lock_type == "new":
        ...
        if lock_engine_id == engine_id or self._engine_alive(lock_engine_id):
            logger.debug("Stack lock is owned by engine %s" % lock_engine_id)
            ...
            rows_affected = db_api.stack_lock_steal(self.stack.id,
                                                    lock_engine_id, engine_id)

            # Either the old_engine_id didn't match (lost race) or
    ...
    def release(self):
        ...
        # This ensures the lock being released is
        rows_affected = db_api.stack_lock_release(self.stack.id, engine_id)
        if rows_affected == 0:
