...
class FakeSubscription(object):
    def __init__(self, name, id=None, ttl=None, subscriber=None, options=None,
                 auto_create=True):
        self.id = id
        self.name = name
        self.ttl = ttl
...
class ZaqarSubscriptionTest(common.HeatTestCase):
    ...
    def test_create(self, mock_client, mock_plugin):
        ...
        fake_subscr = FakeSubscription(subscr.properties['name'],
                                       subscr_id, auto_create=False)
        self.m.StubOutWithMock(self.fc, 'subscription')
        self.fc.subscription(subscr.properties['name'],
                             options={'key1': 'value1'},
        ...
        self.assertEqual(subscr_id, subscr.FnGetAtt('id'))
    ...
    def test_update_replace(self, mock_client, mock_plugin):
        ...
        t = template_format.parse(wp_template)
        t['Resources']['MySubscription']['Properties']['name'] = 'foo'
        resource_defns = template.Template(t).resource_definitions(self.stack)
    ...
    def test_show_resource(self, mock_client, mock_plugin):
        ...
        self.fc.subscription(
            subscr.properties['name'], id=subscr_id,
