...
class DBAPIStackTagTest(common.HeatTestCase):
    ...
    def test_stack_tag_set(self):
        tags = db_api.stack_tag_set(self.ctx, self.stack.id, ['tag1', 'tag2'])
        self.assertEqual(self.stack.id, tags[0].stack_id)
        self.assertEqual('tag1', tags[0].tag)

        tags = db_api.stack_tag_set(self.ctx, self.stack.id, [])
        self.assertIsNone(tags)

    def test_stack_tag_get(self):
        db_api.stack_tag_set(self.ctx, self.stack.id, ['tag1', 'tag2'])
        tags = db_api.stack_tag_get(self.ctx, self.stack.id)
        self.assertEqual(self.stack.id, tags[0].stack_id)
        self.assertEqual('tag1', tags[0].tag)

        tags = db_api.stack_tag_get(self.ctx, UUID1)
        self.assertIsNone(tags)

    def test_stack_tag_delete(self):
        db_api.stack_tag_set(self.ctx, self.stack.id, ['tag1', 'tag2'])
        db_api.stack_tag_delete(self.ctx, self.stack.id)
        tags = db_api.stack_tag_get(self.ctx, self.stack.id)
        self.assertIsNone(tags)
