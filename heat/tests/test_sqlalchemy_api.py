...
class DBAPIStackLockTest(HeatTestCase):
    ...
    def test_stack_lock_create_success(self):
        lock_type, lock_engine_id = create_stack_lock(self.stack.id, UUID1)
        create_stack_lock(self.stack.id, UUID1)
        self.assertEqual(lock_type, "new")
        self.assertEqual(lock_engine_id, UUID1)

    def test_stack_lock_create_fail(self):
        create_stack_lock(self.stack.id, UUID1)
        lock_type, lock_engine_id = create_stack_lock(self.stack.id, UUID1)
        self.assertEqual(lock_type, "existing")
        self.assertEqual(lock_engine_id, UUID1)
