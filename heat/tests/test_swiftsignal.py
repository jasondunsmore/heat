...
class SwiftSignalHandleTest(HeatTestCase):
    ...
    def test_create(self, mock_swift):
        ...
        obj_name = "%s-%s" % (st.name, handle.name)
        ...
        obj_name = "%s-%s" % (st.name, handle.name)
        regexp = ("http://fake-host.com:8080/v1/AUTH_test_tenant/%s"
                  "/%s\?temp_url_sig=[0-9a-f]{40}&temp_url_expires=[0-9]{10}" %
                  (st.id, obj_name))
    ...
    @mock.patch.object(swift.SwiftClientPlugin, '_create')
    def test_no_account_meta_temp_url_key(self, mock_swift):
        st = create_stack(swiftsignalhandle_template)
        handle = st['test_wait_condition_handle']
        ...
        st.create()
        self.assertTrue(mock_swift_object.post_account.called)
        self.assertTrue(st.resources['test_wait_condition_handle'].resource_id)
...
class SwiftSignalTest(HeatTestCase):
    ...
    @mock.patch.object(swift.SwiftClientPlugin, '_create')
    def test_multiple_signals_same_id_complete(self, mock_swift):
        st = create_stack(swiftsignal_template)
        handle = st['test_wait_condition_handle']
    ...
    @mock.patch.object(swift.SwiftClientPlugin, '_create')
    def test_multiple_signals_same_id_timeout(self, mock_swift, mock_clock):
        st = create_stack(swiftsignal_template)
        handle = st['test_wait_condition_handle']
    ...
    @mock.patch.object(swift.SwiftClientPlugin, '_create')
    def test_post_complete_to_handle(self, mock_swift):
        st = create_stack(swiftsignal_template)
        handle = st['test_wait_condition_handle']
    ...
    @mock.patch.object(swift.SwiftClientPlugin, '_create')
    def test_post_failed_to_handle(self, mock_swift):
        st = create_stack(swiftsignal_template)
        handle = st['test_wait_condition_handle']
    ...
    @mock.patch.object(swift.SwiftClientPlugin, '_create')
    def test_data_noid(self, mock_swift):
        st = create_stack(swiftsignal_template)
        handle = st['test_wait_condition_handle']
    ...
    @mock.patch.object(swift.SwiftClientPlugin, '_create')
    def test_data_nodata(self, mock_swift):
        st = create_stack(swiftsignal_template)
        handle = st['test_wait_condition_handle']
    ...
    @mock.patch.object(swift.SwiftClientPlugin, '_create')
    def test_data_partial_complete(self, mock_swift):
        st = create_stack(swiftsignal_template)
        handle = st['test_wait_condition_handle']
        ...
        st.create()
        self.assertEqual(['SUCCESS', 'SUCCESS'], handle.get_status())
        expected = [{'status': 'SUCCESS', 'reason': 'Signal 1 recieved',
                     'data': None, 'id': 1},
                    {'status': 'SUCCESS', 'reason': 'Signal 2 recieved',
                     'data': None, 'id': 2}]
        self.assertEqual(expected, handle.get_signals())

    @mock.patch.object(swift.SwiftClientPlugin, '_create')
    def test_get_status_none_complete(self, mock_swift):
        st = create_stack(swiftsignal_template)
        handle = st['test_wait_condition_handle']
    ...
    @mock.patch.object(swift.SwiftClientPlugin, '_create')
    def test_get_status_partial_complete(self, mock_swift):
        st = create_stack(swiftsignal_template)
        handle = st['test_wait_condition_handle']
    ...
    @mock.patch.object(swift.SwiftClientPlugin, '_create')
    def test_get_status_failure(self, mock_swift):
        st = create_stack(swiftsignal_template)
        handle = st['test_wait_condition_handle']
    ...
    @mock.patch.object(swift.SwiftClientPlugin, '_create')
    def test_getatt_token(self, mock_swift):
        st = create_stack(swiftsignalhandle_template)
        handle = st['test_wait_condition_handle']
    ...
    @mock.patch.object(swift.SwiftClientPlugin, '_create')
    def test_getatt_endpoint(self, mock_swift):
        st = create_stack(swiftsignalhandle_template)
        handle = st['test_wait_condition_handle']
        ...
        expected = ('http://fake-host.com:8080/v1/AUTH_test_tenant/%s/'
                    'test_st-test_wait_condition_handle\?temp_url_sig='
                    '[0-9a-f]{40}&temp_url_expires=[0-9]{10}') % st.id
        self.assertThat(handle.FnGetAtt('endpoint'),
                        MatchesRegex(expected))

    @mock.patch.object(swift.SwiftClientPlugin, '_create')
    def test_getatt_curl_cli(self, mock_swift):
        st = create_stack(swiftsignalhandle_template)
        handle = st['test_wait_condition_handle']
        ...
        st.create()
        self.assertEqual(('CREATE', 'COMPLETE'), st.state)
        expected = ('curl -i -X PUT '
                    '\'http://fake-host.com:8080/v1/AUTH_test_tenant/%s/'
                    'test_st-test_wait_condition_handle\?temp_url_sig='
                    '[0-9a-f]{40}&temp_url_expires=[0-9]{10}\'') % st.id
        self.assertThat(handle.FnGetAtt('curl_cli'), MatchesRegex(expected))

    @mock.patch.object(swift.SwiftClientPlugin, '_create')
    def test_invalid_json_data(self, mock_swift):
        st = create_stack(swiftsignal_template)
        handle = st['test_wait_condition_handle']
    ...
    @mock.patch.object(swift.SwiftClientPlugin, '_create')
    def test_unknown_status(self, mock_swift):
        st = create_stack(swiftsignal_template)
        handle = st['test_wait_condition_handle']
    ...
    @mock.patch.object(swift.SwiftClientPlugin, '_create')
    def test_swift_objects_deleted(self, mock_swift):
        st = create_stack(swiftsignal_template)
        handle = st['test_wait_condition_handle']
    ...
    @mock.patch.object(swift.SwiftClientPlugin, '_create')
    def test_swift_container_deleted(self, mock_swift):
        st = create_stack(swiftsignal_template)
        handle = st['test_wait_condition_handle']
