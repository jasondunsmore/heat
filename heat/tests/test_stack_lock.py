...
class StackLockTest(HeatTestCase):
    ...
    def test_successful_acquire_new_lock(self):
        ...
        db_api.stack_lock_get(mox.IgnoreArg()).AndReturn(None)
    ...
    def test_failed_acquire_new_lock(self):
        self.m.StubOutWithMock(db_api, "stack_lock_get")
        db_api.stack_lock_get(mox.IgnoreArg()).AndReturn(None)

        self.m.StubOutWithMock(db_api, "stack_lock_create")
        db_api.stack_lock_create(mox.IgnoreArg(), mox.IgnoreArg()).AndReturn(0)

        self.m.ReplayAll()

        slock = stack_lock.StackLock(self.context, self.stack)
        exc = self.assertRaises(exception.Error, slock.acquire)
        self.assertIn("failed to create lock", str(exc))
        self.m.VerifyAll()
    ...
    def test_successful_acquire_existing_lock_engine_dead(self):
        ...
        rpc.call(self.context, rpc.make_msg("listening"), timeout=5,
    ...
    def test_successful_release(self):
        ...
        self.m.StubOutWithMock(db_api, "stack_lock_release")
        db_api.stack_lock_release(mox.IgnoreArg()).AndReturn(1)
    ...
    def test_failed_release(self):
        ...
        self.m.StubOutWithMock(db_api, "stack_lock_release")
        db_api.stack_lock_release(mox.IgnoreArg()).AndReturn(0)

        self.m.ReplayAll()

        slock = stack_lock.StackLock(self.context, self.stack)
        slock.acquire()
        exc = self.assertRaises(exception.Error, slock.release)
        self.assertIn("failed to release lock", str(exc))
        self.m.VerifyAll()
