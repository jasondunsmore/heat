    ...
    with session.begin():
        ...
        for raw_template in _get_batch(
            ...
            try:
                ...
                for param_name, param_val in env['parameters'].items():
                    ...
                    schema = param_schemata[param_name]
                    if (schema.type == schema.STRING and
                            type(param_val) is not str):
                ...
                LOG.warn(_LW('Failed to encrypt parameters of raw '
                             'template %(id)d. Encountered the following '
                             'exception: %(exc)s'),
                         {'id': raw_template.id, 'exc': exc})
                continue
        ...
        for resource in _get_batch(
            ...
            try:
                ...
                LOG.warn(_LW('Failed to encrypt properties_data of resource '
                             '%(id)d. Encountered the following exception: '
                             '%(exc)s'), {'id': resource.id, 'exc': exc})
                continue
    ...
    with session.begin():
        ...
        for raw_template in _get_batch(
            ...
            try:
                ...
                LOG.warn(_LW('Failed to decrypt parameters of raw '
                             'template %(id)d. Encountered the following '
                             'exception: %(exc)s'),
                         {'id': raw_template.id, 'exc': exc})
                continue
        ...
        for resource in _get_batch(
            ...
            try:
                ...
                LOG.warn(_LW('Failed to decrypt properties_data of '
                             'resource %(id)d. Encountered the following '
                             'exception: %(exc)s'),
                         {'id': resource.id, 'exc': exc})
                continue
